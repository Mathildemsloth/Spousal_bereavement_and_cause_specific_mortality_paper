...
# Also the sex variable and keep only the bereaved individuals
df <- df %>% 
...
fit_cancer_simple <- lrm(surv_cancer3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_cancer_noexp <- lrm(surv_cancer3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T) 

#model with expenditures as arv.
fit_cancer_arv <- lrm(surv_cancer3 ~ affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient,3) + rcs(average_outpatient,3) +  rcs(average_prescription,3) + rcs(average_primarycare,3) + rcs(average_residential,3) + rcs(average_homecare,3), data = df_train, x = T, y = T)
...
#simple model with age and sex
fit_cv_simple <- lrm(surv_CVD3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_cv_noexp <- lrm(surv_CVD3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T)  

#model with expenditures as arv.
fit_cv_arv <- lrm(surv_CVD3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient, 3) +rcs(average_outpatient, 3) +  rcs(average_prescription, 3) + rcs(average_primarycare, 3) + rcs(average_residential,3) + rcs(average_homecare,3), 
...
#simple model with age and sex
fit_neuro_simple <- lrm(surv_neuro3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_neuro_noexp <- lrm(surv_neuro3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T)  

#model with expenditures as arv.
fit_neuro_arv <- lrm(surv_neuro3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient, 3) +rcs(average_outpatient, 3) +  rcs(average_prescription, 3) + rcs(average_primarycare, 3) + rcs(average_residential,3) + rcs(average_homecare,3), 
            data = df_train, x = T, y = T)
```
...
#simple model with age and sex
fit_dm_simple <- lrm(surv_dm3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_dm_noexp <- lrm(surv_dm3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T)  

#model with expenditures as arv.
fit_dm_arv <- lrm(surv_dm3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient, 3) +rcs(average_outpatient, 3) +  rcs(average_prescription, 3) + rcs(average_primarycare, 3) + rcs(average_residential,3) + rcs(average_homecare,3), 
            data = df_train, x = T, y = T)
```
...
#simple model with age and sex
fit_digestive_simple <- lrm(surv_digestive3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_digestive_noexp <- lrm(surv_digestive3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T)  

#model with expenditures as arv.
fit_digestive_arv <- lrm(surv_digestive3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient, 3) +rcs(average_outpatient, 3) +  rcs(average_prescription, 3) + rcs(average_primarycare, 3) + rcs(average_residential,3) + rcs(average_homecare,3), 
            data = df_train, x = T, y = T)
```
...
#simple model with age and sex
fit_psychiatric_simple <- lrm(surv_psychiatric3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_psychiatric_noexp <- lrm(surv_psychiatric3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T)

#model with expenditures as arv.
fit_psychiatric_arv <- lrm(surv_psychiatric3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient, 3) +rcs(average_outpatient, 3) +  rcs(average_prescription, 3) + rcs(average_primarycare, 3) + rcs(average_residential,3) + rcs(average_homecare,3), 
            data = df_train, x = T, y = T)
```
...
#simple model with age and sex
fit_resp_simple <- lrm(surv_respiratory3 ~  sex + age_bereavement.factor, data = df_train, x = T, y = T)

# model with sociodemographics + number of comorbidities
fit_resp_noexp <- lrm(surv_respiratory3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status, data = df_train, x = T, y = T)  

#model with expenditures as arv.
fit_resp_arv <- lrm(surv_respiratory3 ~  affluence.factor + sex + age_bereavement.factor + children_n.factor + comorbidities_factor + immigration_status + rcs(average_inpatient, 3) +rcs(average_outpatient, 3) +  rcs(average_prescription, 3) + rcs(average_primarycare, 3) + rcs(average_residential,3) + rcs(average_homecare,3), 
            data = df_train, x = T, y = T)
```
...
xg_cancer_recipe <- recipe(surv_cancer3 ~  affluence.factor + sex + age_bereavement.factor + 
...
```{r}
#writing recipe for the formula and making the dummy variables
xg_CVD_recipe <- recipe(surv_CVD3 ~  affluence.factor +                            sex + age_bereavement.factor + 
                           children_n.factor + comorbidities_factor +
                           immigration_status + average_inpatient +
                           average_outpatient + average_prescription +
                           average_primarycare + average_residential +
...
```{r}
#writing recipe for the formula and making the dummy variables
xg_neuro_recipe <- recipe(surv_neuro3 ~  affluence.factor +                            sex + age_bereavement.factor + 
                           children_n.factor + comorbidities_factor +
                           immigration_status + average_inpatient +
                           average_outpatient + average_prescription +
                           average_primarycare + average_residential +
...
```{r}
#writing recipe for the formula and making the dummy variables
xg_dm_recipe <- recipe(surv_dm3 ~  affluence.factor +                            sex + age_bereavement.factor + 
                           children_n.factor + comorbidities_factor +
                           immigration_status + average_inpatient +
                           average_outpatient + average_prescription +
                           average_primarycare + average_residential +
...
```{r}
#writing recipe for the formula and making the dummy variables
xg_digestive_recipe <- recipe(surv_digestive3 ~  affluence.factor +                            sex + age_bereavement.factor + 
                           children_n.factor + comorbidities_factor +
                           immigration_status + average_inpatient +
                           average_outpatient + average_prescription +
                           average_primarycare + average_residential +
...
```{r}
#writing recipe for the formula and making the dummy variables
xg_psychiatric_recipe <- recipe(surv_psychiatric3 ~  affluence.factor +                            sex + age_bereavement.factor + 
                           children_n.factor + comorbidities_factor +
                           immigration_status + average_inpatient +
                           average_outpatient + average_prescription +
                           average_primarycare + average_residential +
...
```{r}
#writing recipe for the formula and making the dummy variables
xg_respiratory_recipe <- recipe(surv_respiratory3 ~  affluence.factor +                            sex + age_bereavement.factor + 
                           children_n.factor + comorbidities_factor +
                           immigration_status + average_inpatient +
                           average_outpatient + average_prescription +
                           average_primarycare + average_residential +
...
# Decision curves
The paper includes decision curves for all causes of death. Here some examples are presented
